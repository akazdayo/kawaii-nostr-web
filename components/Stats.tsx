...
const Stats: React.FC<StatsProps> = ({ posts }) => {
  const hourData: ActivityData[] = Array.from({ length: 24 }, (_, i) => ({ hour: i, count: 0 }));
  ...
  const colors = ['#ffb7b2', '#ff9aa2', '#e2f0cb', '#b5ead7', '#c7ceea', '#ffdac1'];
  ...
  return (
    <div className="w-full h-full bg-white/40 p-6 rounded-3xl border border-white shadow-sm flex flex-col">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400 bg-white px-2 py-1 rounded-full">UTC Time</span>
      </div>
